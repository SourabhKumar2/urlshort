...
`
	t.Run("valid yaml", func(t *testing.T) {
		...
		if parsedYaml[0].Path != "/urlshort" {
			t.Errorf("expected path %v but got %v", "/urlshort", parsedYaml[0].Path)
		}
	})
}
...
`
	t.Run("valid yaml", func(t *testing.T) {
		...
		if err == nil {
			t.Error("Expected error but didn't get the error")
		}
	})
}
...
func TestBuildMap(t *testing.T) {
	...
	t.Run("Build Map", func(t *testing.T) {
		put := buildMap(pathUrls)
		if _, ok := put[pu.Path]; !ok {
			t.Errorf("expected path %v in map but it isn't there", pu.Path)
		}
	})
}
...
type handlerFunc func([]byte, http.Handler) (http.HandlerFunc, error)
...
func createReaderHandler(handler handlerFunc, yaml string) http.HandlerFunc {
	...
	handlerF, err := handler([]byte(yaml), fallbackHandler)
	if err != nil {
		fmt.Println(err)
...
func assertStatus(t *testing.T, resp *http.Response, want int) {
	...
	if got != want {
		t.Errorf("Expected status to be %d, got %d", want, got)
	}
}
